...
savefig("FinalFigs/Ratios_$(k)_Congress_Bootstrap.pdf")
...
p = plot!(1:k,[TA1 TA1], fillrange=[MA1-sc*SA1 MA1+sc*SA1],fillalpha=0.3,color = color1, linewidth = lw, markersize = ms,
    markershape = :circle,markerstrokecolor = color1, label = class1)
savefig("FinalFigs/Affinities_$(k)_Congress_Bootstrap.pdf")


## Plot normalized bias scores
lw = 2.5
ms = 8
gfs = 15
tfs = 15
...
savefig("FinalFigs/NormBias_$(k)_Congress_Bootstrap.svg")
